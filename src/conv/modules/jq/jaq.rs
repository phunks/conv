...
pub fn jq(ui: &mut Ui, editor: &mut Editor) {
    run(ui, editor, &Settings {
        indent: 4,
        tab: true,
        ..Default::default()
    });
...
fn fmt_val(f: &mut Formatter, opts: &PpOpts, level: usize, v: &Val) -> fmt::Result {
    let display = |s| FormatterFn(move |f: &mut Formatter| fmt_str(f, s));
    match v {
...
fn color_from_val(v: &Val) -> Option<Color32> {
    match v {
        Val::Null => color_from_hex("#504945"),
        Val::Bool(_) => color_from_hex("#928374"),
        Val::Int(_) => color_from_hex("#fabd2f"),
        // Val::Float(_) if x.is_finite() => {},
        Val::Float(_) => color_from_hex("#8ec07c"),
        Val::Num(_) => color_from_hex("#d3869b"),
        Val::Str(_) => color_from_hex("#fe8019"),
        // Val::Arr(_) => color_from_hex("#83a598"),
        // Val::Obj(_) if o.is_empty() => {},
        // Val::Obj(_) => color_from_hex("#a89984"),
        _ => None,
    }
}
...
//             indent: get("indent").and_then(4_usize)?,
//             tab: get_bool("tab")?,
//         })
//     }
// }
...
pub fn run(ui: &mut Ui, editor: &mut Editor, settings: &Settings) {
    ...
    let post_value = |y| {
        let s = FormatterFn(|f: &mut Formatter| match &y {
            Val::Str(s) if settings.raw_output => span(f, "string", s),
            y => fmt_val(f, &pp_opts, 0, y),
        });
        //scope.post_message(&s.to_string().into()).unwrap();
        // job.append(&s.to_string(),0.,TextFormat {
        //     font_id: FontId::new(11.5, FontFamily::Monospace),
        //     color: Color32::ORANGE,
        //     ..Default::default()
        // })
    };
    match process(editor, &settings, post_value) {
        Ok(()) => (),
...
fn process(editor: &mut Editor, settings: &Settings, mut f: impl FnMut(Val)) -> Result<(), Error> {
    let (_vals, filter) = parse(&editor.jq.filter, &[]).map_err(Error::Report)?;

    let inputs = read_str(settings, &editor.code);
